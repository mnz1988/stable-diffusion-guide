...
با اسکرول به بالا برگردید و بر روی دکمه نارنجی رنگ بزرگ که برروی آن نوشته **Apply settings** بزنید و بعد **Reload UI** کنار آن را بزنید.
    
حالا میتوانید با داشتن مدل پایه ای که نصب شده است تصاویر را تولید کنید که البته ممکن است زیاد مورد پسند نباشد. من از مدل پایه Runway استفاده میکنم که اگر تمایل داشته باشید میتوانید آن را دانلود و در پوشه مدل ها قرار دهید. حجم این فایل بزرگ است و حدود 8 گیگابایت میباشد: https://huggingface.co/runwayml/stable-diffusion-v1-5/blob/main/v1-5-pruned.ckpt  و برای فاین تیون نیز مناسب است و همچنین بصورت پیشفرض فیلترهای سانسور و حذف خروجی های بزرگسالان در آن وجود ندارد
...
**مدل ها (چک پوینت)** <a name="model"></a>[▲](#index)
   ...
   همانطور که گفته شد مدل ها را چک پوینت نیز می نامند که مغز هوش مصنوعی شما هستند و به هدف تولید انواع مختلفی از تصاویر طراحی شده اند. گزینه های بسیار زیاد و متنوعی از آنها وجود دارد (به خصوص در پلتفرم [civitai](https://civitai.com) ) اما کدام را انتخاب کنیم؟ تعدادی پیشنهاد به شما ارائه میدهم: 
 ...
 **توضیحات VAEها** <a name="vae"></a>[▲](#index)
   ...
   **درخواست ها / Prompts** <a name="prompt"></a>[▲](#index)
   ...
   در اولین تب **txt2img** شما بیشتر تصاویرتان را می سازید. اینجا جاییست که درخواست *prompt* خود و چیزهایی که نمیخواهید در آن باشد (*negative prompt*/درخواست منفی) را وارد میکنید. استیبل دیفیوژن مثل میدجرنی (Midjourney) یا دیگر نرم افزارهای مشابه نیست که فقط از آنها درخواستی کلی برای تولید چیزی کنید، باید به طور واضح آن را مشخص کنید، *خیلی* واضح. اغلب مردم یک پرامپت (درخواست) پیدا میکنند که کارشان را راه می اندازد و از آن استفاده میکنند یا از دیگران برای پیشنهادات کمک می گیرند. اینجا چند نمونه پرامپت و پرامپت منفی شخصی را نشان میدهم:
  ...
  برای دانلود این فایل کوچک و نصب آن از [این لینک](https://huggingface.co/datasets/gsdf/EasyNegative/resolve/main/EasyNegative.safetensors) استفاده کنید و آن را در پوشه `stable-diffusion-webui/embeddings` قرار دهید و اگر برنامه در حال اجراست آن را ریلود *Reload UI* کنید. و هروقت آن کلمه را بنویسید (EasyNegative) عمل میکند.
   ...
   بعد از نوشتن پرامپت ابتدایی یا پایه ای (شبیه پرامپت های بالا که مثال زده شد) خواسته های خود را اضافه میکنیم. مثلا `young woman in a bikini in the beach, full body shot` را اضافه میکنیم. همچنین برای پرامپت منفی نیز چیزهایی که مایل نیستیم در تصویر باشد را اضافه میکنیم...
   ...
   یک تکنیک مهم برای نوشتن پرامپت، تاکید (مثبت) و نفی کردن (تاکید منفی) بر کلمات است. وقتی چیزی را داخل پرانتز () میگذاریم برنامه **وزن** بیشتری در خروجی تولید تصویر برای آن قائل می شود و به هوش مصنوعی میگوید که این قسمت مهمتر است. وزن معمول برای همه چیز برابر با 1 است و هر پرانتز آن را در 1.1 ضرب میکند تا وزن بیشتری بگیرد.(همچنین میتوانید از چندین پرانتر ((())) برای بالا بردن وزن استفاده کنید) 
   ...
   **معیارهای تولید تصویر/Generation parameters** <a name="gen"></a>[▲](#index)
   ...
   بخش **Sampling steps**: اینها تعداد "محاسبات" از پیش تعیین شده را مشخص میکنند. نکته اول این است که همیشه تعداد بالاتر استپ ها به معنی جزئیات و نتیجه بهتر نیست. من معمولا از عدد 30 استفاده میکنم و شما ممکن است دوست داشته باشید از 20 تا 50 را امتحان کنید تا مقدار مناسب و نتیجه دلخواه را برایتان داشته باشد.
   ...
   بخش **Batch Count** و **Batch Size**: بچ سایز یعنی کارت گرافیک چه تعداد تصویر را در یک زمان تولید کند که محدود به میزان VRAM آن است. بچ کانت هم یعنی چندبار این فعالیت تکرار شود. بچ ها وابسته به سید هستند. توضیحات سید را در بخش `seeds`ببینید.
...
بخش های دیگر:
   ...
   - بخش **Tiling**: برای تولید تکستچرهای تکرار شونده و قرار دادن در grid استفاده می شود.
...
معرفی تعدادی از اکستنشن های کاربردی: اگر از کولب این راهنما استفاده میکنید تعداد زیادی از آنها را در حال حاضر دارید در غیر اینصورت میتوانید آنها را خودتان اضافه کنید.
...
همانطور که در [توضیحات قبلی ▲](#gen) گفته شد معمولا بهتر است که از ابعاد 768 برای طول یا عرض تصویر فراتر نروید. به جای آن از `Hires fix` و انتخاب upscaler مناسب و سطح حذف نویز (denoising level) مناسب با آن استفاده کنید تا نتایج بهتری بدست آورید. شما میتوانید upscaler های متعددی را دانلود کنید (نصب مستقیم با کولب) و در فولدر `stable-diffusion-webui/models/ESRGAN` قرار دهید. **Remacri** که در نسخه مربوط به کولب اضافه شده است یک upscaler خوب همه جانبه و عمومی است. 
   ...
   *تعداد قابل توجهی را میتوانید [اینجا پیدا کنید](https://huggingface.co/hollowstrawberry/upscalers-backup/tree/main/ESRGAN) و درصورت تمایل آنها را نصب کنید.
...
به بخش اسکریپت ها/**Scripts** رفته و گزینه **Ultimate SD Upscale** را انتخاب کنید. سپس، پارامترهایی مشابه به این را با اندازه های مورد نظرتان تنظیم کنید و بعد **Type** را روی **Chess** قرار دهید:

![Ultimate upscale parameters](images/ultimate.png)

اگر میزان VRAM کافی دارید، ممکن است تمایل داشته باشید مقدار **Tile width** را همانند **Padding** افزایش دهید. برای مثال، هر دو را دوبرابر کنید. مقدار **Tile height** میتواند همان 0 باشد تا با عرض هماهنگ باشد.
...
به شما اجازه میدهد که یک اسکچ ساده را با کمک پرامپتی که نوشته اید به یک تصویر نهایی تبدیل کنید. این تنها مثالی است که از تصویر نمونه بالا استفاده نمیکند. مشاهده نمایید: 
...
میتوانید یک Lora را روی کامپیوتر شخصی خود نیز آموزش دهید، فقط کافیست که کارت گرافیک شما دارای حداقل 8 گیگابایت VRAM باشد. در اینجا لیستی از چندین منبع آموزشی مختلف را برایتان میگذارم:
